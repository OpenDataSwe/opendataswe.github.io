...
use crate::plot::line_draw::{draw_absolute_plots, PlotOpts};
...
pub struct LineAbsolutePlotProps {
    pub calculation_range: CalculationRange,
    pub base_calculation_data: BaseCalculationData,
}

impl LineAbsolutePlot {
    fn draw(&self, props: &LineAbsolutePlotProps) -> Result<(), String> {
        let element: HtmlCanvasElement = self
            .canvas
            .cast()
            .ok_or_else(|| "Failed to convert noderef to html canvas".to_string())?;
        ...
        drawing_area
            ...
            .map_err(|_e| "Failed line absolute plot drawing area".to_string())?;
        draw_absolute_plots(
            &drawing_area,
...
impl Component for LineAbsolutePlot {
    type Message = PlotMessage;
    type Properties = LineAbsolutePlotProps;
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        html! {
            <div>
                <Checkbox info_html={TOTAL_PERSONLIG_SKATT} checked={self.plot_opts.personal} on_change={on_change_personal} label_text={"Personlig skatt"} input_name={"personal_tax_absolute"} input_id={"personal_tax_absolute"} />
                <Checkbox info_html={TOTAL_SKATTESUMMA} checked={self.plot_opts.total} on_change={on_change_total} label_text={"Totala skatter"} input_name={"total_tax_absolute"} input_id={"total_tax_absolute"} />
                <Checkbox info_html={KVAR_EFTER_SKATT} checked={self.plot_opts.left} on_change={on_change_left} label_text={"Kvar efter skatt"} input_name={"money_left_absolute"} input_id={"money_left_absolute"} />
    ...
    fn rendered(&mut self, ctx: &Context<Self>, _first_render: bool) {
        if let Err(e) = self.draw(ctx.props()) {
            ctx.link().send_message(Self::Message::ChartErr(e));
        }
    }
}
