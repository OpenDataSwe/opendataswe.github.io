...
use crate::plot::line_draw::{draw_absolute_incline, InclinePlotOpts};
...
pub struct LineAbsoluteInclinePlot {
    canvas: NodeRef,
    err: Option<String>,
...
pub struct PlotInclineProps {
    pub calculation_range: CalculationRange,
    pub base_calculation_data: BaseCalculationData,
}

impl LineAbsoluteInclinePlot {
    fn draw(&self, props: &PlotInclineProps) -> Result<(), String> {
        let element: HtmlCanvasElement = self
            .canvas
            .cast()
            .ok_or_else(|| "Failed to cast element to html canvas".to_string())?;
...
impl Component for LineAbsoluteInclinePlot {
    type Message = PlotMessage;
    type Properties = PlotInclineProps;
